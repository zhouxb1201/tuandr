<template>
  <Layout ref="load" class="order-confirm bg-f8">
    ...
    <template v-if="isShowShipping">
      <van-cell title="配送方式" class="cell-panel" v-if="!noExpress && isPickup">
      ...
      <ChoiceAddress :address="address" @select="onAddress" v-show="isShowAddress" />
    </template>
    ...
    <div class="list">
      <van-cell-group class="items card-group-box" v-for="(item,index) in items" :key="index">
        ...
        <van-cell
          title="优惠券"
          :value="item.coupon_name?'已选：'+item.coupon_name:'有 '+item.coupon_num+' 张可用优惠券'"
          class="cell-right-text"
          is-link
        ...
        <van-cell>
          <div class="foot">
            共&nbsp;
            <span>{{shopGoodsAmount(item.goods_list)}}</span>&nbsp;件商品&nbsp;&nbsp;&nbsp;小计：
      ...
      />
      <CellInfoGroup class="card-group-box" :columns="cellInfoGroup" textAlign="right" />
    </div>
    <SubmitBar
        ...
        if (this.is_tax_fee) {
          this.shop_invoice_list.forEach((i_item)=>{
            if(shop_obj.shop_id == i_item.shop_id){
              ...
              shop_obj.invoice.invoice_tax = e.tax_fee[i_item.invoice.invoice_tax_key]
            }
          })
        }
        shop_obj.goods_list = [];
            ...
            $this.items = data.shop.map((e, i) => {
              ...
              if ($this.params.presell_id) {
                e.goods_list.forEach(g => {
                  ...
                  e.presell_info.tailMoney = parseFloat(e.presell_info.final_real_money); // 计算尾款
                  e.presell_info.tailText = '(含运费'+(e.tax_fee?'税费':'')+')';
