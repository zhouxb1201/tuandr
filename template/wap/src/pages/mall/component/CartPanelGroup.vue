<template>
  <van-cell-group class="items card-group-box">
    <van-cell title-class="cell-title">
      ...
      <div class="cell-delivery" @click="showDelivery" v-if="$store.state.config.addons.store">
        <van-icon name="location-o" size="14px" />
        <span class="text">{{deliveryText}}</span>
      </div>
      <div slot="right-icon" v-if="couponList.length">
        <span class="shop-title-rigth e-handle" @click="isShowPopupCoupon = true">领券</span>
      </div>
    </van-cell>
    <van-cell class="goods-item" v-for="(item,index) in items.goods_list" :key="index">
      <div class="info">
        ...
        >
          ...
          <div class="sku-text" slot="desc" v-if="item.sku_name" @click="clickSku(item)">
