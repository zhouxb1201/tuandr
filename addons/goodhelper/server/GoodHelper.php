                        ...
                        if (count($right) == count($v_img_arr)) {//该行所有图片也全在zip解压成功的集合中
                            ...
                            if (count($v_img_arr) == $t) {
                                ...
                                $data = [
                                    ...
                                    'is_hot' => $excelData['是否促销'],
                                    'is_recommend' => $excelData['是否推荐'],
                                    'is_hot' => $excelData['是否包邮'],
...
/*                    //处理商品图片
                    $sheetCount = count($v);
                    $img_name_arr = explode('|', $v[$column]);
                    $imag_name_arr_count = count($img_name_arr);//Excel中一行数据中所包含的图片
                    $right_temp = [];
                    foreach ($img_name_arr as $img) {
                        if (in_array($img,  $zip_error_info['right'])) {
                            $right_arr[$k][] = $img;// 记录正确上传图片,Excel循环删除
                            $t = array_keys($zip_error_info['right'], $img)[0];//查询包含该图片名的$zip_error_info的key
                            $right_temp[] = $zip_error_info['right_path'][$t];//图片名对应的图片URL
                        }
                    }

                    if (count($right_arr[$k]) == $imag_name_arr_count) {
                        if(!@array_combine($key_array, $v)){
                            continue;
                        }
                        $excelData = array_combine($key_array, $v);
...
//            if (stristr($type, $v['type'])) {
    ...
    public function setCategory($category_name = '', $attr_id = 0, $website_id) {
        ...
        try {
            ...
            $cate_3 = $cateModel->getInfo(['category_name' => $categoryArr[2], 'short_name' => $categoryArr[2], 'level' => 3, 'pid' => $category['category_id_2'], 'attr_id' => $attr_id, 'website_id' => $website_id])['category_id'];
            if (!$cate_3) {
                $cateModel = new VslGoodsCategoryModel();
