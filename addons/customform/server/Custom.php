...
class Custom extends BaseService
{
    ...
    public function addCustomFormTag($tag_name,$tag_id)
    {
        ...
        $data = $custom_tag->where('tag_name',$tag_name)->whereOr('id',$tag_id)->find();
        $tag = $custom_tag->where('tag_name',$tag_name)->where('id','<>',$tag_id)->find();
        if ($tag) {
            return -1;
        }

        if(empty($data)){
            $custom_tag->tag_name = $tag_name;
            $custom_tag->create_time = time();
            $custom_tag->website_id = $this->website_id;
            $custom_tag->save();
        } else {
