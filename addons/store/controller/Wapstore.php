...
class Wapstore extends baseStore
{
    ...
    */
    ...
    {
        $type = request()->post("type", 1);//1:已上架商品的一级分类  2:仓库中商品的一级分类
        if ($type == 1) {
            //已上架商品的一级分类
            $condition = array(
                'website_id' => $this->website_id,
    ...
    */
    ...
    public function addCart() {
        ...
        if ($bar_code) {
            $sku = $sku_model->getInfo(['bar_code' => $bar_code, 'website_id' => $this->website_id, 'store_id' => $this->store_id], 'sku_id');
        ...
        $sku_info = $sku_model->getInfo(['sku_id' => $sku_id, 'store_id' => $this->store_id, 'website_id' => $this->website_id],'*');
        if (!$sku_info) {
            return json(['code' => -1, 'message' => '商品不存在']);
        }
        ...
        $shop_id = $this->instance_id;
        $storeId = $sku_info['store_id'];

        $picture_id = $pic;
        $result = $storeServer->addcartForStore($shop_id, $goods_id, $sku_id, $num, $picture_id, $storeId);
        if ($result > 0) {
    ...
    public function printTicket() {
        ...
        $storeServer = new storeServer();
        $storeInfo = $storeServer->storeDetail($store_id);
        $assistantInfo = $storeServer->assistantDetail($assistant_id, $store_id);
        $price = 0;
        $num = 0;
        $orderInfo = '<CB>' . $storeInfo['store_name'] . '</CB><BR>';
        $orderInfo .= '收银员:' . $assistantInfo['assistant_name'] . '　　　　机号:01<BR>';
        $orderInfo .= '时间:' . date('Y-m-d H:i:s') . '<BR>';
        $orderInfo .= '--------------------------------<BR>';
        $orderInfo .= '名称　　　　　 单价  数量 小计<BR>';
        foreach ($order_info['order_goods'] as $row) {
            $orderInfo .= $row['goods_name'] . '   ' . $row['price'] . '   ' . $row['num'] . '  ' . number_format($row['price'] * $row['num'], 2) . '<BR>';
            $price += number_format($row['price'] * $row['num'], 2);
            $num += $row['num'];
        }
        $orderInfo .= '--------------------------------<BR>';
        $orderInfo .= '件数:' . $num . '　　　　　     应付:' . number_format($price, 2) . '<BR><BR>';
        $orderInfo .= '<C>谢谢惠顾，欢迎下次光临</C>';
