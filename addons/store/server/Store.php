...
class Store extends BaseService
{
    ...
    public function getStoreGoods($page_index, $page_size, $condition, $o2o = 0)
    {
        ...
        foreach ($goods_info as $key => $val) {
            $arr = $albumPictureModel->Query(['pic_id' => $val['picture']], 'pic_cover')[0];
    ...
    public function getCartList($cart_id, &$msg = '')
    {
        ...
        foreach ($cart_lists as $k => $v) {
            ...
            if ($v->sku->stock <= 0) {
                $cart->destroy(['cart_id' => $v->cart_id]);
                unset($cart_lists[$k]);
                $msg .= $goods_name . "商品该sku规格库存不足，已移除" . PHP_EOL;
                continue;
            }
            if ($v->goods->state != 1) {
                $this->cartDelete($v->cart_id);
...
//        return $max_buy;
     ...
     */
    public function addCartForStore($shop_id, $goods_id, $sku_id, $num, $picture, $store_id) {
        ...
        $stock = $this->getSkuBySkuid($sku_id, $store_id);
    ...
    public function getCart($store_id = 0, $is_deduction = 0, $discount = 0, $manual_amout = 0) {
        ...
        $deduction_data = [];
        $return_point_data = [];
        $point_deductio = $order_business->pointDeductionOrder($returnData['cart_list'], $is_deduction, 2);
