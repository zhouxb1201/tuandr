...
class Miniprogram extends baseMiniProgram
{
    ...
    public function testerQrCode()
    {
        ...
        if (stripos($result,'errcode')) {
            //使用默认图片
            return request()->domain().'/wap/static/images/mappqr.jpg';
        } else {
           //压缩图片
            // 上传云端
    ...
    {
        $type = request()->post('type', 1);//1发布直播 2不发布直播
        // 查询template_id
...
//        $ext_data['pages'] = Config::get('mp_route');
    ...
    public function saveCustom()
    {
        ...
        if ($is_admin) {
            $template_data =  str_replace('"goodstype":"0"', '"goodstype":"2"',$template_data);//C端默认goodstype=2 店铺
        }
