define(['utilAdmin','jquery-ui'], function (util) {
    var modal = {
        id: 0,
        type: 1,            //  页面类型    默认1首页
        navs: {},
    ...
    modal.initEditor = function (scroll) {
        ...
        $('[data-toggle="selectImgFooter"]').unbind('click').click(function(){
            ...
            util.mpFooterDialog(function(content){
                var path2="/public/admin/images/wpFooterIcon/";
                path2 += content;
                path2 += ".png";
        ...
        $('#selectGoods').unbind('click').click(function () {
            ...
            util.goodsDialog('url:'+__URL(ADMINMAIN + '/config/modalGoodsList&t='+(new Date()).getTime()+'&goods_type='+goods_type),function(data){
        ...
        $('#selectShop').unbind('click').click(function () {
            ...
            util.shopDialog('url:'+__URL('/admin/addons/execute/addons/shop/controller/shop/action/modalshoplist&t='+(new Date()).getTime()),function(data){
                modal.items[itemid].data = data.data;
	...
	modal.initNavs = function () {
		...
		// navgroup[9]  为积分商城组件
		var navgroup = {
            0: ['title','line','blank','picture','goods','richtext', 'listmenu'],
			...
			7: [],
			8: [],
			9: ['search', 'menu', 'notice', 'banner', 'picturew'],
		};
    ...
    modal.save = function (preview) {
        ...
        $.ajax({
            type :'post',
            url : __URL(ADMINMAIN + '/addons/execute/addons/miniprogram/controller/miniprogram/action/saveCustom'),
            data : {
                'id':modal.id,
