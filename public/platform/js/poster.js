define(['util', 'contextmenu', 'tpl'], function (util, contextmenu, tpl) {
    // design.js
...
//拖放程序
    ...
    function bindEvents(obj) {
        ...
        if (type == 'nickname' || type == 'img' || type == 'title' || type == 'marketprice' || type ==
            'productprice' || type == 'ptime' || type == 'words' || type == 'wx' || type == 'level' || type == 'phone' || type=='goods-img') {
            rs.Scale = false;
        }
        new Drag(obj, {
            Limit: true,
    ...
    function bind(obj) {
        var imgset = $('#imgset'),
            ...
            qrset = $('#qrset');
            wordset = $('#wordset');
        imgset.hide(), nameset.hide(), qrset.hide(),wordset.hide();
        imgset.find(".goods-pic").val('');
        clearTimers();
        var type = obj.attr('type');
        if (type == 'img') {
            imgset.show();
        ...
        } else if (type == 'nickname' || type == 'title' || type == 'marketprice' || type ==
            'productprice' || type == 'ptime' || type == 'wx' || type == 'level' || type == 'phone') {

            nameset.show();
            var color = obj.attr('color') || "#000";
            var size = obj.attr('size') || "14";
            var input = nameset.find('input:first');
            var inputColor = nameset.find('.input-group-addon:first');
            ...
            nametimer = setInterval(function () {
                ...
                obj.attr('size', namesize.val()).find('.text').css('font-size',
                    namesize.val() + "px");
            }, 10);
            ...
            var color = obj.attr('color') || "#000";
            var size = obj.attr('size') || "14";
            var input = wordset.find('input:first');
            var inputColor = wordset.find('.input-group-addon:first');
            var namesize = wordset.find('#namesize1');
            var picker = wordset.find('.sp-preview-inner');
            var contents = obj.attr('content') || "文字内容";
            var contentVal = wordset.find('#wordContent');
            contentVal.val(contents);
            input.val(color);
            inputColor.css("background-color", color);
            namesize.val(size.replace("px", ""));
            picker.css({
            ...
            wordtimer = setInterval(function () {
                obj.attr('color', input.val()).find('.text').css('color', input.val());
                obj.attr('size', namesize.val()).find('.text').css('font-size',namesize.val() + "px");
                obj.attr('content', contentVal.val()).find('.text').html(contentVal.val());
            }, 10);
        }
    }
    ...
    function getImgUrl(val) {
        ...
        if (val.indexOf('http://') == -1) {
            // val = "http://pic.vslai1.com/" + val;
        }
        return val;
    }

    function initPoster(data) {
        if (!util.isEmpty(data)) {
            $('#poster .bg').attr('src', data.bg);
            $('#poster .bg').attr('data-id', data.bg_id);
            $('input[name=bg]').val(data.bg)
            if (!util.isEmpty(data.items)) {
                $.each(data.items, function (k, v) {
                    var img_html = "";
                    if (v.type == 'qr' || v.type == 'head' || v.type == 'img' || v.type == 'thumb' || v.type == 'goods-img') {
                        img_html = '<img src="' + v.img + '" />';
                    } else {
                        img_html = '<div class=text style="color: ' + v.color + '; font-size: ' + v.size + 'px;">' + v.string + '</div>';
                    }
                    var obj = $('<div class="drag" type="' + v.type + '" index="' + v.index +
                        '" style="' + v.style + '" color="' + v.color + '" size="' + v.size + '" src="' + v.img + '">' + img_html +
    ...
    function posterData() {
        var poster_data = {}
        poster_data.bg = $("#poster .bg").attr('src')
        poster_data.bg_id = $("#poster .bg").data('id');
        // console.log(poster_data.bg)
        poster_data.items = []
        $('#poster .drag').each(function () {
            ...
            item.color = $this.attr('color')
            if (item.type == 'img' || item.type == 'qr' || item.type == 'head' || item.type == 'thumb' || item.type == 'goods-img') {
                item.img = $this.children('img').attr('src')
            } else {
                item.string = $this.children('.text').html()
            }
            item.from_top = $this.css('top').replace('px', '')
            item.from_left = $this.css('left').replace('px', '')
            item.width = $this.css('width').replace('px', '') || 80
            item.height = $this.css('height').replace('px', '') || 80
            poster_data.items.push(item)
        })
    ...
    // 参照底图的图片空间
    $('body').on('click', '[data-toggle="editPosterPicture"]', function () {
        var _this = $(this);
        util.pictureDialog(_this, false, function (content) {
            _this.parent().prev().val(content.path[0]);
            $('.bg').attr('src', content.path[0]);
            $('.bg').attr('data-id', content.id[0]);
        })
    });
    //参照底图清除
    ...
    // "图片"的图片空间
    $('body').on('click', '[data-toggle="editPosterPicture1"]', function () {
        var _this = $(this);
        util.pictureDialog(_this, false, function (content) {
            _this.parent().prev().val(content.path[0]);
            // $('.bg').attr('data-id',content.id[0]);
    ...
    $('.btn-com').click(function () {
        ...
        var img = "";
        if (type == 'qr') {
            img = '<img src="/public/static/images/poster/poster_code.png" style="width:100%;height:100%" />';
        } else if (type == 'head') {
            img = '<img src="/public/static/images/poster/poster_head.png" style="width:100%;height:100%" />';
        } else if (type == 'img' || type == 'thumb' || type == 'goods-img') {
            img = '<img src="/public/static/images/poster/poster_pic.png" style="width:100%;height:100%" />';
            ...
            img = '<div class="text">Y-m-d H:i:s</div>';
        } else if (type == 'words') {
            img = '<div class="text">文字内容</div>';
        } else if (type == 'wx') {
            img = '<div class="text">微信号</div>';
        } else if (type == 'level') {
            img = '<div class="text">等级</div>';
        } else if (type == 'phone') {
            img = '<div class="text">手机号</div>';
        }
        var index = $('#poster .drag').length + 1;
        if(type == 'nickname' || type == 'productprice' || type == 'marketprice' || type == 'title'){
            var obj = $('<div class="drag" type="' + type + '" index="' + index +
                '" style="z-index:' + index + '" color="#000" size="14">' + img +
                '<div class="rRightDown"> </div><div class="rLeftDown"> </div><div class="rRightUp"> </div><div class="rLeftUp"> </div><div class="rRight"> </div><div class="rLeft"> </div><div class="rUp"> </div><div class="rDown"></div></div>'
            );
        }else{
            var obj = $('<div class="drag" type="' + type + '" index="' + index +
