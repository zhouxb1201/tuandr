define(["jquery", "utilAdmin", "colorpicker", "bootstrap", "jconfirm",'jquery-ui'], function ($, utilAdmin, colorpicker) {
    var adminAddgood = {};
    /**
     * 微商来 - 专业移动应用开发商!
     * ========================================================= Copy right
     ...
     * 
     */
    adminAddgood.goodsSkuCreate = function (obj) {
        //默认实物商品
        var goods_type = 1;
        var valid_type = 0;
        var is_wxcard = 0;
         ...
         * @param is_selected
         */
        function addOrDeleteSpecObj(spec_name, spec_id, spec_value_name, spec_value_id, spec_show_type, spec_value_data, is_selected, spec_value_data_src) {
            var is_have = 0;
            if(is_selected){
                if(spec_show_type == '3'){
                    selectedImage = 1;
        ...
        function createTable() {
            ...
            if ($specObj.length == 0) {
                ...
                $("#txtProductSalePrice").removeAttr("readonly");
                $("#txtProductMarketPrice").removeAttr("readonly");
                $("#txtProductCostPrice").removeAttr("readonly");
                $("#txtProductSalePrice").prop("required", true);
                $("#item_no").removeAttr("readonly");
                $("#txtProductCount").prop("required", true);
            } else {
                $("#stock_table").show();
                if ($("#txtProductCount").attr("readonly") != "readonly") {
                    $("#txtProductCount").attr("readonly", "readonly").removeAttr('required');
                    $("#txtProductCount").parents(".form-group").removeClass("has-error");
                }
                if ($("#txtProductCostPrice").attr("readonly") != "readonly") {
                    $("#txtProductCostPrice").attr("readonly", "readonly").removeAttr('required');
                    $("#txtProductCostPrice").parents(".form-group").removeClass("has-error");
                }
                if ($("#txtProductMarketPrice").attr("readonly") != "readonly") {
                    $("#txtProductMarketPrice").attr("readonly", "readonly").removeAttr('required');
                    $("#txtProductMarketPrice").parents(".form-group").removeClass("has-error");
                }
                if ($("#txtProductSalePrice").attr("readonly") != "readonly") {
                    $("#txtProductSalePrice").attr("readonly", "readonly").removeAttr('required');
                    $("#txtProductSalePrice").parents(".form-group").removeClass("has-error");
                }
                if ($("#item_no").attr("readonly") != "readonly") {
                    $("#item_no").attr("readonly", "readonly").removeAttr('required');
                    $("#item_no").parents(".form-group").removeClass("has-error");
                }
            }
            ...
            th_html += '<th class="th-stock">库存（件）</th>';
            th_html += '<th class="th-code">商品货号</th>';
            th_html += '</tr>';
            ...
            for (var i = 0; i < $sku_array.length; i++) {
                ...
                html += '<td class="w15"><input type="number" min="1"  autocomplete="off" mustNum="true" required data-visi-type="prices_1" name="stock_num" class="onblur form-control" value="' + $temp_Obj[child_id_string]["stock_num"] + '"/></td>';
                ...
                for (var m = 0; m < $specObj[i]["value"].length; m++) {
                    ...
                    if (show_types == 2) {
                        ...
                        if (src == "" || src == undefined) {
                            src = ADMINIMG + "/goods_sku_add.png";
                        } else {
                            src = __IMG(src);
                        }
...
// 颜色选择器
        ...
        function getStoreList(){
            ...
            $.ajax({
                ...
                success: function (data) {
                	...
                	if(data.code==-1005){
                		$("#store_unselect").html('<div style="margin-top:100px;text-align: center;"><label class="prompts">无法添加，店铺应用未授权</label></div>');
                	}else if((data.total_count == 0 || storeUnselect =='') && data.addon_status.is_store==1){
                		$("#store_unselect").html('<div style="margin-top:100px;text-align: center;"><label class="prompts">没有营业中的门店，前往<a href="'+__URL(ADDONSADMINMAIN+'addStore')+'" class="text-primary addPrompt" data-type="store" target="_blank">创建</a></label></div>');
                	}else if(data.addon_status.is_store==0){
                		$("#store_unselect").html('<div style="margin-top:100px;text-align: center;"><label class="prompts">店铺应用未开启</label></div>');
                	}
               }
            });
        }
        ...
        $('body').on('click','.addPrompt',function(){
            ...
            utilAdmin.alert('是否已添加完成？', function () {
            	if(type=='store'){
            		getStoreList();
            	}
            })
        ...
        $("#store_select").on('click', "input[name='select_store_id[]']", function () {
            if ($("#store_unselect label").hasClass("prompts")) {
                $("#store_unselect").empty();
            }
        ...
        $(function () {
            if (parseInt($("#goodsId").val()) > 0) {
                $("#goods_attribute_id").attr("data-flag", 2);//标识 0：表示添加商品，1：表示编辑商品，商品分类为0,2：表示编辑商品，商品分类不为0
                ...
                $("#goods_attribute_id").attr("data-flag", 0);//标识 0：表示添加商品，1：表示编辑商品，商品分类为0,2：表示编辑商品，商品分类不为0
                getGoodsSpecListByAttrId($("#goods_attribute_id").val());
            }
            ...
            $('#spec_list').on('click', '.specItemValue', function () {
                ...
                if(parseInt(spec_show_type) == '2' || parseInt(spec_show_type) == '3'){
                    if(!checkHasChooseShowType(spec)){
                        if(parseInt(spec_show_type) == '3'){
                            utilAdmin.message('图片类型只能选择一个','danger');
                            return false;
                        }
                        if(parseInt(spec_show_type) == '2'){
                            utilAdmin.message('颜色类型只能选择一个','danger');
                            return false;
                        }
                    }//颜色图片只能各自选择一个
                }
                ...
                if ($this.is(":checked")) {
                    addOrDeleteSpecObj(spec_name, spec_id, spec_value_name, spec_value_id, spec_show_type, spec_value_data, 1, spec_value_data_src);
                } else {
                    addOrDeleteSpecObj(spec_name, spec_id, spec_value_name, spec_value_id, spec_show_type, spec_value_data, 0, spec_value_data_src);
                }
                createTable();
            });
            //规格改版js
            ...
            $("body").on('click', '.goods-value-sure', function () {
                ...
                if (val == '') {
                    if (show_type < 10) {
                        utilAdmin.message('请填写规格值名称', 'danger');
                    } else if (show_type == 10) {
                        utilAdmin.message('请填写规格名称', 'danger');
                    } else if (show_type == 11) {
                        utilAdmin.message('请填写属性名称', 'danger');
                    } else if (show_type == 12 || show_type == 13) {
                        utilAdmin.message('请填写属性值名称', 'danger');
                    } else if (show_type == 'edit') {
                        utilAdmin.message('名称不能为空', 'danger');
                    }
                    return false;
                }
                ...
                if (show_type < 10) {
                        ...
                        success: function (data) {
                            if (data['code'] < 0) {
                                ...
                                switch (parseInt(show_type)) {
                                    ...
                                    case 2:
                                        //颜色
                                        html += '<article class="goods-sku-items"><label class="checkbox-inline"><input type="checkbox" name="label_list_' + spec_id + '" value="" class="specItemValue" data-spec_id="' + spec_id + '" data-spec_name="' + spec_name + '" data-spec_value_id="' + data.code + '" data-spec_value_data="#000000"><span class="goods-sku-items-name" data-status="edit_spec_value">' + val + '</span></label> <span><input type="color"  name="spec_value_data" class="colorpicker"></span></article>';
                                        break;
                                    case 3:
                                        //图片
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data["code"] > 0) {
                                var html = '<tr class="attrbute_list"  type="1" id="' + data['code'] + '"><input type="hidden" name="attr_sort" value="1"><td><span class="goods-sku-items-name J-attr_name" data-status="edit_attr">' + val + '</span><a class="ml-2 J-deleteAttr" data-id="' + data['code'] + '" href="javascript:void(0);"><i class="icon icon-delete"></i></a> </td><td class="text-left"><article class="goods-sku-items"><input type="text" class="form-control w-200 js-attribute-text" name="attr_value" data-attribute-value="' + val + '" data-attribute-value-id="' + data['code'] + '"></article></td></tr>';
                                _this.parents('.last-tr').before(html);
                                addDom.show();
                                _this.parents('.value-add-div').remove();
                            } else {
                                utilAdmin.message("添加失败", 'danger');
                            }
                        }
                    });
                    ...
                    $.ajax({
                        type: "post",
                        url: __URL(ADMINMAIN + '/goods/addattributevaluename'),
                        data: {
                            'attr_value_name': val,
        ...
        function editSpecName(event) {
            ...
            $('.batchSet').on('click', function () {
                ...
                html += '</div></div></form>'
                utilAdmin.confirm('批量修改' + batch_text, html, function () {
                    var val;
                    ...
                    if (!val || val == '') {
                        utilAdmin.message(batch_text + '不能为空');
                        return false
                    } else if (val > maxNum && batch_type !== 'code') {
                        utilAdmin.message('价格最大为 ' + maxNum);
                        return false
                    }
            ...
            $('body').on('change', "#stock_table tbody tr td input", function () {
                ...
                success: function (data) {
                    ...
                    if (attribute_list.length > 0) {
                        ...
                        attribute_list.forEach(function (item, i) {
                            attribute_list_html += '<tr class="attrbute_list" id="' + item.attr_value_id + '" type="' + item.type + '"><input type="hidden" name="attr_sort" value="' + item.sort + '"><td><span class="goods-sku-items-name J-attr_name" data-status="edit_attr">' + item.attr_value_name + '</span>'; 
                            if(item.shop_id > 0){
                                attribute_list_html += '<a class="ml-2 J-deleteAttr" data-id="' + item.attr_value_id + '" href="javascript:void(0);"><i class="icon icon-delete"></i></a>';
                            }
                            attribute_list_html += '</td><td class="text-left">';
...
//                                    attribute_list_html += '<a href="javascript:void(0);" class="goods-value-add" data-show-type="13"><i class="icon icon-add1 mr-04"></i>添加属性值</a>';
            ...
            $(".goods-sku-block-" + spec.spec_id).find('.goods-sku-items ').each(function () {
                switch (parseInt(spec.show_type)) {
                    ...
                    case 2:
                        ...
                        $(this).append('<input type="color" class="colorpicker" value="">');
                        break;
                    case 3:
                        //图片
                        $(this).find('.spec-img-box').remove();
        ...
        function setCategory() {
            var url = __URL(ADMINMAIN + "/goods/selectcategory");
        ...
        $("input[name='is_distribution']").on("change", function () { 
            ...
            if (val == 1) {
                ...
                $("input[name=recommend_type]:eq(0)").prop("checked", "checked");
                $("#distribution_input").removeAttr("checked");
            } else {
                //不参与分销 隐藏all
        ...
        $('body').on('click', '.J-selectCategory', function () {
            if ($('#category_id_1').val() > 0) {
                utilAdmin.alert('切换分类后会重置规格与属性，是否继续操作？', function () {
                    setCategory();
                });
            } else {
                setCategory();
        ...
        $('.other-category').on('click', function () {
            ...
            utilAdmin.confirm('选择分类', 'url:' + url, function () {
                ...
                for (var i = 0; i < arr.length; i++) {
                    if (arr[i] == id) {
                        utilAdmin.message('该分类已被选择', 'danger');
                        return false;
                    }
                }
        ...
        function getBindAttr(cid) {
            $.ajax({
                ...
                url: __URL(ADMINMAIN + '/goods/getbindingattr'),
                success: function (data) {
        ...
        $('#curr-list').on('click', '.tryView', function () {
            ...
            if(tvType == 0){
                ...
                html+='</form>';
                utilAdmin.confirm('试学',html,function(){
                ...
                utilAdmin.alert('确定取消试学吗？',function(){
                    that.html('试学');
                    that.data('type','0');
        ...
        $('#curr-list').on('click','.viewDel',function(){
            var that = $(this);
            utilAdmin.alert('确定要删除该内容吗？',function(){
        ...
        $('body').on('click','[data-toggle="singlePicVideo"]',function(){
            var _this = this;
            utilAdmin.picVideoDialog(_this,false,function(data){
                if(data.type[0] == 1){
                    var viewTypeText ='视频';
                    var viewType ='1';
                }else{
        ...
        $("#btnSave").on('click', function () {
            ...
            try {
                ...
                if (err == '1') {
                    utilAdmin.message('请填写全部销售价', 'danger');
                    $('.add_tab1 li:eq(1) a').tab('show');
                    return false;
                }
                if (err == '2') {
                    utilAdmin.message('请填写全部市场价', 'danger');
                    $('.add_tab1 li:eq(1) a').tab('show');
                    return false;
                }
                if (err == '3') {
                    utilAdmin.message('请填写全部库存', 'danger');
                    $('.add_tab1 li:eq(1) a').tab('show');
                    return false;
                }
                ...
                if(lengths == 0){
                    ...
                    try {
                        $('input[name="payContentName"]').each(function () {
                            ...
                            if (err == '1') {
                                utilAdmin.message('请填写全部付费内容名称', 'danger');
                                $('.add_tab1 li a.pay_content').tab('show');
                                return false;
                            }
                    }
        ...
        utilAdmin.validate("#form1", function (form) {
            if (parseInt($("#maxLimitBuy").val()) !=0 && (parseInt($("#singleLimitBuy").val()) > parseInt($("#maxLimitBuy").val()))) {
                ...
                utilAdmin.message('单次限购不能大于最大限购', 'danger');
                return false;
            }
            var type = 0;
            img_id_arr = "";// 商品主图
            ...
            if (productViewObj.store_list == '' && storeStatus == 1 && goods_type == 0) {
                utilAdmin.message('请选择核销门店', 'danger');
                return false;
            }
            ...
            flag = true;
            if (productViewObj.goods_type == 0 && productViewObj.card_switch == 1) {
                $("#saveTips").removeClass("hide");
            }
            $.ajax({
                ...
                success: function (res) {
                    $("#saveTips").addClass("hide");
                    var url = __URL(ADMINMAIN + "/goods/goodslist");
                    ...
                    if (res['code'] > 0) {
                        ...
                        utilAdmin.message((goodsId > 0) ? "修改成功" : "添加成功", 'success', function () {
                            location.href = __URL(ADMINMAIN + "/goods/goodslist");
                        });
                    } else {
                        utilAdmin.message((goodsId > 0) ? "修改失败" : "添加失败", 'danger');
                        flag = false;
        ...
        $('body').on('click', '.J-refresh', function () {
            ...
            $.ajax({
                url: __URL(ADMINMAIN + "/goods/getShippingFeeList"),
                type: "post",
                success: function (data) {
                    if (data.length > 0) {
        ...
        $('body').on('click', '.J-deleteAttr', function () {
            var $this = $(this);
            var attr_value_id = $this.attr('data-id');
            utilAdmin.alert('删除不可逆，确定删除？',function(){
                $.ajax({
                    url: __URL(ADMINMAIN + "/goods/deleteGoodsAttr"),
                    type: "post",
        ...
        $('body').on('click', '.shows_type', function () {
            ...
            if (now_shop_type == 1 || now_shop_type == 0) {
                html += 'checked=""';
            }
            ...
            html += '</div></div></form>';
            var typeFlag = true;
            utilAdmin.confirm('设置展示类型', html, function () {
                if(typeFlag){
                    typeFlag = false;
        ...
        $('body').on('click', '[data-toggle="specPicture"]', function () {
            ...
            utilAdmin.spec_picDialog(_this, false, function (content) {
