define(['util','jquery-ui'], function (util) {
    ...
    modal.init = function (params) {
        modal.default_version = params.default_version
    ...
    modal.initData = function(data){
        ...
        modal.popupAdv = params.popupAdv || modal.defaultPopupAdvData();
        if(modal.type == 1 || modal.type == 4){
            modal.tabbar = params.tabbar;
            modal.copyright = params.copyright;
            modal.initFoot();
        }else{
            $("#foot").hide()
        }
    ...
    modal.initPage = function (initE) {
        ...
        $("#page").css({'color': modal.page.navbarcolor});
        $("#page").css({'background-color': modal.page.navbarbackground});
        $("#view").find(".drag").removeClass("selected");
	...
	modal.initDefaultItems = function(){
		...
		if(modal.type == 5){
			$.each(modal.items, function (itemid, item) {
				if(item.id == 'commission_fixed' && (!item.params || !item.params.styletype)){
					item.params = {}
					item.style = {}
					item.params.styletype = '1'
		...
		if(modal.type == 4){
			var newItems = {};
			$.each(modal.items, function (itemid, item) {
				if(item.id == 'member_fixed' && (!item.params || !item.params.styletype) && item.id != "member_bind_fixed" && item.id != "member_assets_fixed" && item.id != "member_order_fixed"){
					$.each(modal.defaultMemberItems(),function(member_item_id,member_item){
						newItems[member_item_id] = member_item
					})
				}else{
			...
			if(newItems['M0_member_assets']){
				// 根据应用开启情况显示相关应用数据
				var arr = ['balance','points']
				if(modal.addonsIsUse.coupontype){
					arr.push('coupontype')
				}
				if(modal.addonsIsUse.giftvoucher){
					arr.push('giftvoucher')
				}
				if(modal.addonsIsUse.store){
					arr.push('store')
				}
				// if(modal.addonsIsUse.blockchain){
				// 	arr.push('blockchain')
				// }
				// 处理旧数据存在问题
				var itemsData = newItems['M0_member_assets']['data']
				$.each(modal.defaultMemberItems()['M0_member_assets']['data'],function(key,item){
					var itemsDataKey = itemsData[key]
					if(itemsDataKey && itemsDataKey.key == item.key){
						itemsData[key] = itemsDataKey
					}else{
						itemsData[key] = item
					}
					itemsData[key].no_addons = arr.indexOf(item.key) == -1 ? '1' : '0'
				})
			}
	...
	// 设置弹窗广告
	modal.setPopupAdv = function(){
		util.confirm('弹窗广告设置','url:'+__URL(PLATFORMMAIN + '/config/modalPopupAdv?t='+(new Date()).getTime()),function(e){
			modal.popupAdv = this.$content.find('#advData').data('set')
		},'large',function(){
			this.$content.find('#advData').data('set',modal.popupAdv)
		})
	};
    // 加载默认数据
    ...
    // 初始化initFoot
    modal.initFoot = function(initType){
        if (typeof(initType) === 'undefined') {
            initType = false
        }
        if(!modal.tabbar){
            modal.tabbar = modal.defaultTabbar();
        }
        if(!modal.copyright){
            modal.copyright = modal.defaultCopyright();
            modal.copyright.params.readonly = modal.default_version
        }else{
            if(modal.default_version == '1'){
                modal.copyright.params.linkurl = 'http://m.vslai.com'
                modal.copyright.params.src = modal.attachurl +'copyright-logo.png'
                modal.copyright.params.text = '微商来提供技术支持'
                modal.copyright.params.showlogo = '1'
                modal.copyright.style.showtype = '0'
            }
            modal.copyright.params.readonly = modal.default_version
        }
        ...
        if(initType == 'tabbar'){
            ...
            $("#tabbar").html(tpl("tpl_show_tabbar", modal.tabbar));

        }else if(initType == 'copyright'){

            $("#copyright").html(tpl("tpl_show_copyright", modal.copyright));
    ...
    modal.initEditor = function (scroll) {
        ...
        $(".colorpicker2").each(function(){
            var elm = this;
            util.colorpicker2(elm, function(color){});
        });
        // 重置颜色
        ...
        // 重置颜色(黑白)
        $('.btn-reset1').unbind('click').click(function(){
            var color = $(this).data('color')
            $(this).parent().find('.sp-preview-inner,.colorpicker2').css('backgroundColor',color).val(color).trigger('propertychange')
        })
        // 滑块
        ...
        // 小程序底部选择图片
        $('[data-toggle="selectImgFooter"]').unbind('click').click(function(){
            var _this = $(this);
            var cimg = _this.parents('.item').find('input'+_this.data('input'));
            var pimg = _this.parents('.item').find('img'+_this.data('img'));
            util.mpFooterDialog(function(content){
                var path2="/public/platform/images/wpFooterIcon/";
                path2 += content;
                path2 += ".png";
                cimg.val(path2).change();
                pimg.attr('src',path2);
            })
        ...
        $('[data-toggle="selectUrl"]').unbind('click').click(function(){
            ...
            util.linksMinDialog(function(data){
                elm.val(data.params).change();
            }, 'mini&template_type='+ modal.type)
        })
        ...
        // 选择CPS商品
        $('#selectCpsGoods').unbind('click').click(function () {
            var itemid = modal.selected;
            var session = new util.Storage('session');
            var goods_type = modal.items[itemid].params.goodstype;
            var type = modal.type;
            session.setItem('goods_data_itemid',itemid);
            util.cpsGoodsDialog('url:'+__URL(PLATFORMMAIN+ '/shop/modalCpsGoodsList&t='+(new Date()).getTime()+'&goods_type='+goods_type+'&type='+type),function(data){
                modal.items[itemid].data = data.data;
                modal.initItems(itemid);
                modal.initEditor(false);
            },function(){
        ...
        $('#selectShop').unbind('click').click(function () {
            ...
            util.shopDialog('url:'+__URL('/platform/addons/execute/addons/shop/controller/shop/action/modalshoplist&t='+(new Date()).getTime()),function(data){
                modal.items[itemid].data = data.data;
                session.setItem('shop_data_'+itemid,JSON.stringify(data));
                modal.initItems(itemid);
                modal.initEditor(false);
            })
        })
        ...
        $("#editor").find(".diy-bind").bind('input propertychange change', function () {
            ...
            } else if(itemid == 'tabbar'){
                if (bindchild) {
                    modal.tabbar[bindparent][bindchild][bind] = value;
                    modal.initFoot('tabbar');
                }

            } else if(itemid == 'copyright'){
                if (bindchild) {
                    modal.copyright[bindchild][bind] = value
                    modal.initFoot('copyright');
                }
	...
	modal.initNavs = function () {
		...
		// navgroup[9]  为积分商城组件
		var navgroup = {
            0: ['title','line','blank','picture','goods', 'listmenu','richtext'],
            1: ['search', 'menu', 'notice', 'banner', 'picturew','customer'],
			2: ['search', 'menu', 'notice', 'banner', 'picturew'],
			3: [],
			4: ['menu', 'notice'],
			5: ['menu', 'notice'],
			6: ['search', 'menu', 'notice','banner', 'picturew'],
			7: [],
			8: [],
    ...
    modal.save = function (preview) {
        ...
        if(modal.page.navbarcolor=="#000000"){
            modal.page.navbarcolor="black"
        }
        if(modal.page.navbarcolor=="#ffffff"){
            modal.page.navbarcolor="white"
        }
        ...
        // console.log(222);
        if(modal.page.title=='请输入标题'){
            if (modal.type == 1) {
                modal.page.title = "商城首页"
            }
            if (modal.type == 2) {
                modal.page.title = "店铺首页"
            }
        ...
        // console.log('data数据==>',modal.data);
        // console.log('tabbar数据==>',modal.tabbar);
        // console.log('copyright数据==>',modal.copyright);
        // console.log('提交数据==>',JSON.stringify(modal.data))
        // return
        $.ajax({
            type :'post',
            url : __URL(PLATFORMMAIN + '/addons/execute/addons/miniprogram/controller/miniprogram/action/saveCustom'),
            data : {
                'id':modal.id,
                'template_data':modal.data,
                'tabbar':modal.tabbar,
                'copyright':modal.copyright
            },
	...
	// 默认弹窗广告数据
	modal.defaultPopupAdvData = function(){
		return {
			advshow:'0',
			advimg:modal.attachurl+'adv-1.jpg',
			advlink:'',
			advrule:'0'
		}
	};
		...
		return {
			data: {
				C0123456789101: {
					...
					normal: modal.attachurl + 'home-normal-1.png',
					active: modal.attachurl + 'home-active-1.png'
				},
				C0123456789102: {
					text: '分类',
					path: 'pages/goods/category',
					normal: modal.attachurl + 'category-normal-1.png',
					active: modal.attachurl + 'category-active-1.png'
				},
				C0123456789103: {
					text: '店铺街',
					path: 'pages/shop/list',
					normal: modal.attachurl + 'shop-normal-1.png',
					active: modal.attachurl + 'shop-active-1.png'
				},
				C0123456789104: {
					text: '购物车',
					path: 'pages/mall/cart',
					normal: modal.attachurl + 'cart-normal-1.png',
					active: modal.attachurl + 'cart-active-1.png'
				},
				C0123456789105: {
					text: '会员中心',
					path: 'pages/member/centre',
					normal: modal.attachurl + 'member-normal-1.png',
					active: modal.attachurl + 'member-active-1.png'
				}
			}
		}
	};
		...
		return {
			...
			"M0_member_assets": {
				...
				"data":{
					...
					"C0_coupontype":{
						"no_addons":modal.addonsIsUse.coupontype?'0':'1',
						"key":'coupontype',
						"name":'优惠券',
						"text":'优惠券',
						"is_show":'1',
					...
					"C0_giftvoucher":{
						"no_addons":modal.addonsIsUse.giftvoucher?'0':'1',
						"key":'giftvoucher',
						"name":'礼品券',
						"text":'礼品券',
						"is_show":'1',
					...
					"C0_store":{
						"no_addons":modal.addonsIsUse.store?'0':'1',
						"key":'store',
						"name":'消费卡',
						"text":'消费卡',
						"is_show":'1',
					...
					"C0_blockchain":{
						"no_addons":modal.addonsIsUse.blockchain?'0':'1',
						"key":'blockchain',
						"name":'数字钱包',
						"text":'数字钱包',
						"is_show":'1',
	...
	modal.getNavs = function () {
		modal.navs = {
			...
			popupAdv:{
				name: '弹窗广告',
				params: modal.defaultPopupAdvData(),
				style:{}
			},
            customer:{
                name: '客服中心',
                params: {
                    imgurl:modal.attachurl+'customer.png'
                },
                style:{}
            }
		}
