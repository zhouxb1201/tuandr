define(['utilAdmin','jquery-ui'], function (util) {
    ...
    modal.init = function (params) {
        modal.realm_ip = params.realm_ip
        ...
        if(modal.type == 3){
            ...
            $.each(modal.items, function (itemid, item) {
                if(item.id == 'detail_fixed'){
                    ...
                    $.each(modal.defaultDetailItems(),function(detail_itemid,detail_item){
                        newItems[detail_itemid] = detail_item;
    ...
    modal.initEditor = function (scroll) {
        ...
        $('#selectGoods').unbind('click').click(function () {
            ...
            util.goodsDialog('url:'+__URL(ADMINMAIN+ '/config/modalGoodsList&t='+(new Date()).getTime()),function(data){
    ...
    modal.initNavs = function () {
        ...
        var navgroup = {
			...
			3: ['detail_banner','detail_info','detail_promote','detail_specs','detail_service','detail_delivery','detail_shop','detail_desc'],
			4: ['menu', 'notice'],
            5: ['menu','notice'],
    ...
    modal.save = function (preview) {
        ...
        if(preview){
            modal.data.copyright = modal.copyright
            modal.data.tabbar = modal.tabbar
            var data = JSON.stringify(modal.data)
            $.dialog({
                title: '',
                animation: 'top',
                closeAnimation: 'bottom',
                animateFromElement: false,
                content: '<iframe id="wap-iframe" style="width:100%;height:600px;border:1px solid #e8e8e8;" src="' + modal.realm_ip + '/wap/preview"></iframe>',
                bgOpacity: 0.1,
                backgroundDismiss: true,
                onContentReady:function(e){
                    var iframe = document.getElementById('wap-iframe').contentWindow;
                    iframe.postMessage({custom:data},modal.realm_ip); 
                }
            })
            return false
        }
        ...
        $.ajax({
            type :'post',
            url : __URL(ADMINMAIN + '/config/saveCustomTemplate'),
            data : {
                'id':modal.id,
            ...
            "M012345678906":{
                ...
                data:{
                    C0123456789101: {
                        title: '7天无理由退货',
                        desc:'商城所有商品均为正品。',
                        imgurl:modal.attachurl + 'goodsicon-sendfree.png',
                    },
                },
            },
            "M012345678907":{
