...
//            $goods_packet_list[$k]['express_code'] = $v_packet['express_code'];
    ...
    public function orderCreate() 
    {
        ...
        $cart_from = $post_data['cart_from'] ?: 0;//是平台购物车还是门店购物车,1平台购物车,2门店购物车
...
//                    $v['anchor_id'] = $keys+1;
        ...
        foreach ($post_data['shop_list'] as $shop) {
            ...
            if (empty($post_data['address_id']) && $post_data['shipping_type']==2 && !$has_store && empty($shop['card_store_id'])) {
            ...
            if ($order_type) {
                ...
                foreach ($order_data['order'] as $shop_id => $v) {
                    foreach ($v['sku'] as $sku => $sku_list) {
                        ...
                        // debugLog($order_data['shop'][$shop_id]['shop_should_paid_amount'], '==>is_store<==');
                ...
                debugLog($seckill_shop_order_data, '普通秒杀1===> ');
            ...
            foreach ($return_data['shop'] as $kk => $vv) {
                ...
                if ($order_data['pay_type'] == 5) {  //余额支付
                    $order_info['pay_money'] = 0;
                        //                    $order_info['user_platform_money'] = $vv['total_amount'] + $invoice_tax;
                    $order_info['user_platform_money'] = $vv['total_amount'];

                } else {
                    //                $order_info['pay_money'] = $vv['total_amount']  + $invoice_tax;
...
/*                $invoice_goods_detail = '';
     ...
     * 创建订单
     */
    public function orderCreate_bak() 
    {
        $order_service = new OrderService();
        $order_business = new OrderBusiness();
        $user_model = new UserModel();
...
//            echo '<pre>';print_r($order_data);exit;
        ...
        foreach ($order_data['order'] as $shop_id => $v) {
            ...
            $order_info['order_from'] = $order_from;
            //开始组装金额相关
            $invoice_tax = $vv['invoice_tax'] ?: 0;//税费
            if ($order_data['pay_type'] == 5) {  //余额支付
                $order_info['pay_money'] = 0;
                $order_info['user_platform_money'] = $vv['total_amount'] + $invoice_tax;
            } else {
...
//                p($sku_info);exit;
                if (!empty($sku_info['presell_id'])) {
                    ...
                    if($presell_goods_info){
                        ...
                        $order_info['final_money'] = $presell_goods_info['allmoney']*$sku_info['num'] - $order_info['pay_money'] + $sku_info['shipping_fee'];
                        $order_info['shipping_fee'] += $sku_info['shipping_fee'];
                    }
                }
                //计时/次商品
...
//            p($order_data);exit;
        ...
        if ($order_id > 0) {
            if (Session::get('order_tag') == 'cart' && $cart_from == 1) {
                $sku_id_array = [];
                if ($post_data['shop_list']) {
                    foreach ($post_data['shop_list'] as $keys => $values) {
                        foreach ($values['goods_list'] as $k => $v) {
                ...
                $delete_cart_condition['sku_id'] = ['IN', $sku_id_array];
                $order_service->deleteCartNew($delete_cart_condition);
            } elseif (Session::get('order_tag') == 'cart' && $cart_from == 2) {
                $sku_id_array = [];
                if ($post_data['shop_list']) {
                    foreach ($post_data['shop_list'] as $keys => $values) {
                        foreach ($values['goods_list'] as $k => $v) {
            ...
            foreach ($post_data['shop_list'] as $keys => $values) {
                foreach ($values['goods_list'] as $k => $v) {
                    ...
                    //商品明细 （名字 价格/ 分类 价格）
                    $goodsCategoryName = $goods_service->getGoodsCategoryNameByGoodId($v['goods_id']);
                    $invoice_goods_detail .= $v['goods_name'];//eg 冬日棉
                    $invoice_goods_category .= $goodsCategoryName;//eg 服装>上衣
                }
                if (getAddons('invoice', $this->website_id, $this->instance_id)) {
                    if (!isset($values['invoice']['type'])) {
                        continue;
...
//            Log::write($order_id);
     ...
     * 创建门店订单
     */
    public function StoreOrderCreate_bak()
    {
        $order_service = new OrderService();
        $order_business = new OrderBusiness();
        ...
        if($post_data['shop_list']){
            foreach ($post_data['shop_list'] as $keys => $values) {
                ...
                foreach ($values['goods_list'] as $k => $v) {
                ...
                } elseif (!empty($values['card_store_id'])) {
                    //计时计次商品
                    foreach ($values['goods_list'] as $k => $v) {
                        if ($v['presell_id']) {
                            $presell_id = intval($v['presell_id']);
                    ...
                    foreach ($values['goods_list'] as $k => $v) {
                        ...
                        if (intval($values['coupon_id'])) {
                            $rdata1['coupon_id'] = intval($values['coupon_id']);
                        }
                        if (intval($v['seckill_id'])) {
                            $rdata1['seckill_id'] = intval($v['seckill_id']);
                        }
                        $rdata1['sku_id'] = intval($v['sku_id']);
                        $rdata1['num'] = intval($v['num']);

                        array_push($new_sku, $rdata1);
                    }
                }
            }
        }
        ...
        $group_id = $post_data['group_id'] ? intval($post_data['group_id']) : '';
        $msg = '';
        $payment_info = $goods_service->storePaymentData($new_sku,$msg, $record_id, $group_id,$presell_id);
        
        $storeServer = getAddons('store', $this->website_id, $this->instance_id) ? new Store() : '';
        foreach ($post_data['shop_list'] as $shop) {
            $shop_id = $shop['shop_id'];
            $has_store = getAddons('store', $this->website_id, $this->instance_id) ? $storeServer->getStoreSet($shop_id)['is_use'] : 0;
            if($has_store && empty($shop['store_id']) && $post_data['address_id']){
                $has_store = 0;
            }
            if ($post_data['shipping_type'] == 2 && $has_store && !$shop['store_id'] && empty($shop['card_store_id'])) {
                return ['result' => false, 'message' => '没有选择门店'];
        ...
        foreach ($payment_info as $shop_id => $shop_info) {
            ...
            foreach ($shop_info['goods_list'] as $sku_id => $sku_info) {
                ...
                if (!empty($shop_info['full_cut']) &&
                    ...
                    if(!empty($address['district'])){
                        ...
                        $tempgoodsc[$sku_info['goods_id']]['count'] = $temp_goodsc[$sku_info['goods_id']]['count'];
                        $tempgoodsc[$sku_info['goods_id']]['goods_id'] = $temp_goodsc[$sku_info['goods_id']]['goods_id'];
                        $payment_info[$shop_id]['goods_list'][$sku_id]['shipping_fee_total'] = $goodsExpress->getGoodsExpressTemplate($tempgoodsc, $address['district'])['totalFee'];
                    }
                    continue;
                }
                if (empty($temp_goods[$sku_info['goods_id']])) {
            ...
            $payment_info[$shop_id]['point_deductio'] = $point_deductio;
            $deduction_data[] = $point_deductio;
        ...
        //判断下 预售 拼团 秒杀等是否超时或者其他的限制  post_data
        // $calculate_result = $order_service->calculateCreateOrderDataTesy($return_data['shop']);
        $calculate_result = $order_service->calculateCreateStoreOrderDataTesy($post_data); 
        if ($calculate_result['result'] === false) {
            return json(['code' => -1, 'message' => $calculate_result['message']]);
        }
        if ($calculate_result['result'] === -2) {
        ...
        if ($order_type) {
            ...
            $order_data['order_from'] = $order_from;
            $order_data['website_id'] = $this->website_id;
            $seckill_shop_order_data = serialize([$uid => $return_data['shop'],$uid+1 => $calculate_result,$uid+2 => $payment_info,$uid+3 => $out_trade_no,$uid+4 => $this->website_id,$uid+5 => $ip,$uid+6 => $order_from,$uid+7 => $shipping_type,$uid+8 => $is_deduction]);
            ...
            foreach ($return_data['shop'] as $k => $money) {
                $kill_invoice_tax = $money['invoice_tax'] ?: 0;//税费
                $money['total_amount'] += $kill_invoice_tax;
                $all_should_paid_amount += $money['total_amount'];
            } 
            // foreach ($order_data['shop'] as $k => $money) {
            //     $all_should_paid_amount += $money['shop_should_paid_amount'];
        ...
        foreach ($return_data['shop'] as $kk => $vv) {
            ...
            //计算渠道商的金额 因为要将渠道商的金额分离，加到渠道商的账户里面
            if ($calculate_result['shop'][$shop_id]['shop_discount_amount'] == 0) {
                $order_info['channel_money'] = 0;
            } else {
                $order_info['channel_money'] = ($calculate_result['shop'][$shop_id]['shop_channel_amount'] / $calculate_result['shop'][$shop_id]['shop_discount_amount']) * $vv['total_amount'];
            }
            ...
            foreach ($vv['goods_list'] as $ke => $va) {
                ...
                if (!empty($va['presell_id'])) {
                    ...
                    if($presell_goods_info){
                        ...
                        $order_info['final_money'] = $presell_goods_info['allmoney']*$va['num'] - $order_info['pay_money'] + $order_info['shipping_fee'];
            ...
            //提交处理
            $order_business = new OrderBusiness();
            $order_id = $order_business->storeOrderCreateNew($order_info);
...
//                            $is_store = true;
            ...
            $redis_key = getcwd();
            $redis_seckill_order_key = 'vslai_seckill_order_for_store_'.$redis_key;
            // 获取支付编号
            $out_trade_no = $order_service->getOrderTradeNo();
            $order_data['out_trade_no'] = $out_trade_no;
            $order_data['ip'] = $ip;
            $order_data['create_time'] = time();
            $order_data['website_id'] = $this->website_id;
            $order_data['instance_id'] = $this->instance_id;
            $order_data['uid'] = $this->uid;
            $order_data['is_seckill'] = 1;
            $order_data['order_data'] = $order_data;
            $order_data['post_data'] = $post_data;
            $order_data['calculate_result'] = $calculate_result;
            $order_data['return_data'] = $return_data;
            $order_data['payment_info'] = $payment_info;
            //$return_data, $post_data, $order_data, $calculate_result, $payment_info
//            $order_data['']
            $seckill_shop_order_data = json_encode($order_data);
            debugLog($seckill_shop_order_data, '门店秒杀1==> ');
//            var_export($seckill_shop_order_data);exit;
            $is_redis_data = $redis->rpush($redis_seckill_order_key, $seckill_shop_order_data);
            //组支付需要的数据
            $all_should_paid_amount = 0;
            foreach ($return_data['shop'] as $k => $money) {
        ...
        $message = $this->subStoreOrderCreate($order_data, $return_data, $post_data, $calculate_result, $payment_info);
        return json($message);

    }
    public function subStoreOrderCreate($order_data, $return_data, $post_data, $calculate_result, $payment_info)
    {
        $user_model = new UserModel();
        $order_service = new \data\service\Order();
        ...
        // 获取支付编号
        $out_trade_no = $order_service->getOrderTradeNo();
        $website_id = $this->website_id = $this->website_id ?: $order_data['website_id'];
        $instance_id = $this->instance_id = $this->instance_id ?: $order_data['instance_id'];
        if($order_data['is_seckill'] == 1){
            $ip = $order_data['ip'];
            $out_trade_no = $order_data['out_trade_no'];
            $website_id = $order_data['website_id'];
            $instance_id = $order_data['instance_id'];
            $this->uid = $order_data['uid'];
        }
        $buyer_info = $user_model::get($this->uid);
        $shipping_time = time();
        $address_id = $return_data['address_id'] ? $return_data['address_id'] : '';
        if ($address_id) {
            $member = new Member();
            $address = $member->getMemberExpressAddressDetail($address_id,$this->uid);
        }
        //组装订单信息 准备写入数据表 --》 start
        foreach ($return_data['shop'] as $kk => $vv) {
            ...
            foreach ($vv['goods_list'] as $ke => $va) {
                ...
                if (!empty($va['presell_id'])) {
                    ...
                    if ($presell_goods_info) {
                        //预售商品 查看是否自提 自提门店是否开启 待处理 商品service可能已经处理
                        $v['sku'][$va['sku_id']]['price'] = $presell_goods_info['allmoney'];
                        $v['sku'][$va['sku_id']]['discount_price'] = $presell_goods_info['allmoney'];
                        $out_trade_no2 = $order_service->getOrderTradeNo();
                        $order_info['out_trade_no_presell'] = $out_trade_no2;
            ...
            if ($post_data['order_type']) {
                ...
                $order_info['order_type'] = $order_data['is_seckill'] == 1 ? 6 : 1;
            }
            if ($post_data['order_type'] == 2) {
                $order_info['order_type'] = 2;
            }
            ...
            $order_info['invoice_tax'] = isset($vv['invoice_tax']) ? $vv['invoice_tax'] : 0;
            $order_info['invoice_type'] = isset($vv['invoice_type']) ? $vv['invoice_type'] : 0;
            ...
            if ($order_id > 0 && is_numeric($order_id)) {
                if ($order_info['pay_money'] > 0) {
                    ...
                    $timeout = date('Y-m-d H:i:s', $order_info['create_time'] + ($shopConfig['order_buy_close_time'] * 60));
                    runhook('Notify', 'orderCreateBySms', array('order_id' => $order_id, 'time_out' => $timeout));
                    runhook('Notify', 'emailSend', ['website_id' => $website_id, 'shop_id' => $instance_id, 'order_id' => $order_id, 'notify_type' => 'user', 'time_out' => $timeout, 'template_code' => 'create_order']);
                }
                $order_model = new VslOrderModel();
