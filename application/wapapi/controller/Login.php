...
class Login extends Controller
{
    ...
    public function __construct()
    {
        ...
        } elseif (!in_array($action, ['oauthlogin', 'callback'])) {
    ...
    public function oauthLogin()
    {
        ...
        if ($type == 'WCHAT') {
            ...
            # 解密信息
                ...
                if ($mp_result['result'] == false) {
                    debugFile($mp_result, '解密失败', 9);
                }
                if (!$mp_result['result']) {
    ...
    public function oauthAppLogin()
    {
        ...
        if ($type == 'QQLOGIN') {
            ...
            if ($result['code'] == USER_NBUND) {
                ...
                // 绑定新用户
                $uid = $member->registerMember($extend_code, '', '', '', '', '', '', '', '', $unionid, '', '', $app_wx_openid, $mall_port,$nick_name);//联合登录 //0-无来源 1-微信公众号 2-PC 3-移动h5 4-小程序 5-app
                if ($uid > 1) {
                    $oa_data['type'] = 4;//app
...
//                        $this->redirect($redirect);
    ...
    public function register()
    {
        ...
        $retval = $member->registerMember($extend_code, '', $password, '', $mobile, '', '', '', '', '', '', '', '', '', $mall_port);
