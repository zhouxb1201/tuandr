...
use addons\goodhelper\controller\Goodhelper;
    ...
    public function goodsReviewsList()
    {
        try {
            ...
            foreach ($review_data['data'] as $k => $v) {
                ...
                $user_img = $member_service->getMemberImage($v["uid"]);
                $review_data['data'][$k]['user_img'] = $user_img ? getApiSrc($user_img) : '';
            }
...
//        }
    ...
    public function goodsList()
    {
        ...
        // 0自营  1全平台 2店铺
        if (isset($goods_type) && $goods_type == 0) {
            $condition['ng.shop_id'] = 0;
        } elseif ($goods_type == 2 && $this->is_shop) {
            //店铺商品
...
//                    $activity_num = $is_bargain['bargain_sales'];
    ...
    public function cartEditSkuOrNum()
    {
        ...
        if ($cart_goods_list) {
            ...
            if ($store_id) {
                $payment_info = $goods_service->storePaymentData($cart_goods_list);
            } else {
        ...
        if ($sku_list) {
            ...
            if (getAddons('store', $this->website_id, $this->instance_id)) {
                ...
                if (count($sku_list) > 1) {
                    $store_str = '';
                    foreach ($sku_list as $k => $v) {
                        $goods_id = $goodsSkuModel->Query(['sku_id' => $v, 'website_id' => $this->website_id], 'goods_id')[0];
