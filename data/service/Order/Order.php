    ...
    public function orderCreateNew(array $order_info)
    {
        ...
        try {
            ...
            if($order_info['order_type'] == 12){
                $res_order_goods = $order_goods->addOrderGoodsForStore($order_id, $order_info['sku_info'], 0, $order_info['buyer_id'], $order_info['website_id'],$order_info['order_type'], $order_info['pay_money'], $order_info['order_from'], $order_info['invoice_tax']);
            }else{
                if(!$order_info['order_types']){
                    $order_info['order_types'] = '';
                }
                $res_order_goods = $order_goods->addOrderGoodsNew($order_id, $order_info['sku_info'], 0, $order_info['buyer_id'], $order_info['website_id'],$order_info['order_type'], $order_info['pay_money'], $order_info['order_from'], 0, $order_info['invoice_tax'], $order_info['order_types']);
            }
...
//                            $price = $sku_info['num'] * ($sku_info['price'] + $sku_info['shipping_fee']);
     ...
     * @return int|mixed
     */
    public function storeOrderCreateNew(array $order_info)
    {
        $this->order->startTrans();
        $account_flow = new MemberAccount();
        $couponServer = getAddons('coupontype', $this->website_id) ? new CouponServer() : '';
...
//            }
            ...
            // 添加订单项
            $order_goods = new OrderGoods();
            $res_order_goods = $order_goods->addOrderGoodsNew($order_id, $order_info['sku_info'], 0, $order_info['buyer_id'], $order_info['website_id'], $order_info['order_type'], $order_info['pay_money'], $order_info['order_from'], $order_info['store_id'],$order_info['invoice_tax']);
            if (!($res_order_goods > 0)) {
                $this->order->rollback();
                return $res_order_goods;
            }
