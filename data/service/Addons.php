    ...
    public function getModuleList($search_text = '') {
        ...
        foreach ($addonsCategory as $key => $category) {
            ...
            foreach ($aAddons as $k1 => $vAddons) {
                ...
                $module = Db::table("sys_module sm")->join('sys_addons sa', $condition, 'left')->where(['sa.id' => $vAddons])->field('sm.*,sa.name,sa.logo,sa.description,sa.logo_small,sa.logo_often,sa.status,sa.up_status,sa.is_value_add,sa.id,sa.sort as ssort')->find();
