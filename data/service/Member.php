...
class Member extends User
{
     ...
     */
    public function registerMember($extend_code,$user_name, $password, $email, $mobile, $user_qq_id, $qq_info, $wx_openid, $wx_info, $wx_unionid,$mp_open_id = '', $pcwx_open_id = '', $app_wx_openid = '', $user_from = 0, $mall_port = 0,$nickname='')
    {
        try{
            $this->website_id = $this->website_id ?: Session::get('shopwebsite_id');
            //$user_from
            $res = parent::add($user_name, $password, $email, $mobile, 0, 1, $user_qq_id, $qq_info, $wx_openid, $wx_info, $wx_unionid, 0,$this->website_id,'',$mp_open_id, $pcwx_open_id, $app_wx_openid, $mall_port);//0-无来源 联合登录 (1-公众号 2-小程序 3-移动H5  4-PC  5-APP)
            if ($res > 0) {
                // 获取默认会员等级id
...
//                $this->loginNew(['mp_open_id' => $mp_open_id, 'website_id' => $this->website_id]);
    ...
    public function bUserInfo($type,$uid,$password,$mobile,$info,$unionid,$pcwx_open_id){
        $user = new UserModel();
    ...
    public function getMemberList($page_index = 1, $page_size = 0, $condition = '', $order = '', $field = '*')
    {
        ...
        foreach ($result['data'] as $k => $v) {
            ...
            $result['data'][$k]['order_num'] = $member_account->getShopSaleNumSum(['is_deleted' => 0, 'buyer_id' => $v['uid'],'website_id' => $this->website_id]);
            $result['data'][$k]['order_money'] = $member_account->getMemberSaleMoney(['is_deleted' => 0,'buyer_id' => $v['uid'],'website_id' => $this->website_id]);
