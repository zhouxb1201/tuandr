...
//                unset($condition['mall_port']);//去掉端口查一下
     ...
     */
    public function add($user_name='', $password, $email, $mobile, $is_system, $is_member, $qq_openid, $qq_info, $wx_openid, $wx_info, $wx_unionid, $instance_id = 0, $website_id = 0, $port = '', $mp_open_id = '', $pcwx_open_id = '', $app_wx_openid = '', $mall_port = 0, $pic = '', $nickname = '')
    {
        $where = [];
        if ($is_system) {
            $where['port'] = $port;
...
//                unset($condition['mall_port']);
     ...
     * @param $condition
     */
    public function rewriteOauthWqOfSession($condition)
    {
        ...
        debugLog(Session::get(), '=======》会员全部1《============ ');
        debugLog($oauthWq, 'oauthWq1=> ');
        debugLog($user_info, 'oauthWq2=> ');
        // 如果有手机号说明不用该session值
