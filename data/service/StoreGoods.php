...
class StoreGoods extends BaseService
{
    ...
    public function getGoodsList($page_index, $page_size, $condition, $field, $order = '')
    {
        ...
        foreach ($goods_info as $key => $val) {
            ...
            $arr = $albumPictureModel->Query($condition, 'pic_cover')[0];
            $goods_info[$key]['goods_img'] = $arr;
            unset($arr);
        }
            ...
            foreach ($order_info as $k => $v) {
                ...
                if ($v['promotion_money'] + $v['coupon_money'] > 0) {
                    ...
                    foreach ($order_info[$k]['order_item_list'] as $k1 => $v1) {
                        //查询商品图片
                        $albumPictureModel = new AlbumPictureModel();
                        $order_info[$k]['order_item_list'][$k1]['picture'] = $albumPictureModel->Query(['pic_id' => $v1['goods_picture']], 'pic_cover')[0];
                        //获取状态名
        ...
        if ($status == 0) {
            ...
            if ($gift_voucher_records_info) {
                foreach ($gift_voucher_records_info as $k => $v) {
                    ...
                    $albumPictureModel = new AlbumPictureModel();
                    $gift_voucher_records_info[$k]['gift_picture'] = $albumPictureModel->Query(['pic_id' => $gift_picture], 'pic_cover')[0];
            ...
            if ($card_info) {
                foreach ($card_info as $k => $v) {
                    ...
                    foreach ($card_list as $key => $val) {
                        $albumPictureModel = new AlbumPictureModel();
                        $card_list[$key]['card_picture'] = $albumPictureModel->Query(['pic_id' => $v['goods_picture']], 'pic_cover')[0];
