<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:77:"/www/wwwroot/www.tuandr.com/addons/presell/template/platform/presellList.html";i:1583811696;}*/ ?>


<div class="mb-20">
    <a href="<?php echo __URL('platform/Menu/addonmenu?addons=addpresell'); ?>" class="btn btn-primary"><i class="icon icon-add1"></i>
        添加预售</a>
...
<script id="presell_list" type="text/html">
    ...
    <tr>
        ...
        <td class="operate fs-0 operationLeft">
                ...
                <div class="link-pos">
                    <div class="link-arrow">
                        ...
                        <div class="flex link-flex">
                            ...
                            <div class="flex-1">
                                <div class="mb-04"><img
                                        src="<%__URL('PLATFORM_MAIN/goods/getGoodsDetailQr')+'?goods_id='+item.goods_id +'&qr_type=1&wap_path=/wap/goods/detail/'%>"
            ...
            <%if item.status == 2%>
            <a href="<?php echo __URL('platform/Menu/addonmenu?addons=editpresell&type=edit&id=<%item.id%>'); ?>" class="btn-operation">编辑</a>
            <%/if%>
            <%if item.status == 1%>
            <a href="javascript:void(0)" id="" class="btn-operation close_presell">关闭</a>
            <%/if%>
            <a href="<?php echo __URL('platform/Menu/addonmenu?addons=orderrecord&id=<%item.id%>'); ?>" class="btn-operation">记录</a>
            <a href="<?php echo __URL('platform/Menu/addonmenu?addons=editpresell&id=<%item.id%>'); ?>" class="btn-operation">详情</a>
            <%if item.status == 3%>
    ...
    require(['util', 'tpl'], function (util, tpl) {
        ...
        $('body').on('click','.del_presell',function () {
            ...
            util.alert('删除？', function () {
                $.ajax({
                    type: "post",
                    url: "<?php echo __URL('platform/addons/execute/addons/presell/controller/Presell/action/del_presell'); ?>",
                    data: {"id": id},
                    dataType: "json",
        ...
        $('body').on('click','.close_presell',function () {
            ...
            util.alert('关闭？', function () {
                $.ajax({
                    type: "post",
                    url: "<?php echo __URL('platform/addons/execute/addons/presell/controller/Presell/action/close_presell'); ?>",
                    data: {"id": id},
                    dataType: "json",
                    success: function (data) {
                        if (data["code"] > 0) {
        ...
        function LoadingInfo(page_index) {
            ...
            $.ajax({
                type: "post",
                url: "<?php echo __URL('platform/Menu/addonmenu?addons=presellList'); ?>",
                data: {
