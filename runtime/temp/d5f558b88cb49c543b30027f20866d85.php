<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:38:"template/platform/Order/editPrice.html";i:1591330112;}*/ ?>
<form class="form padding-15" id="adjust_price_modal">
    <table class="table v-table table-auto-center">
        ...
        <tbody>
        <?php if(is_array($order_goods_list) || $order_goods_list instanceof \think\Collection || $order_goods_list instanceof \think\Paginator): if( count($order_goods_list)==0 ) : echo "" ;else: foreach($order_goods_list as $k=>$goods): ?>
        <tr data-sku-id=<?php echo $goods['sku_id']; ?>>
            <td>
                <div class="media text-left">
                    ...
                    <div class="media-body max-w-300">
                        ...
                        <div class="small-muted line-2-ellipsis">
                            <?php if(is_array($goods['spec']) || $goods['spec'] instanceof \think\Collection || $goods['spec'] instanceof \think\Paginator): if( count($goods['spec'])==0 ) : echo "" ;else: foreach($goods['spec'] as $k_spec=>$spec): ?>
            ...
            <td>
                <?php echo $goods['actual_price']; ?>
            </td>
            <td>
                <?php echo $goods['num']; ?>
            </td>
            <td>
                <input type="number" id="<?php echo $goods['order_goods_id']; ?>" data-actual-price="<?php echo $goods['actual_price']; ?>" class="form-control number-form-control J-edit-price"
                       min="0" step="0.1">
            </td>
        </tr>
    ...
    <div class="form-group">
        <p class="form-control-static">运费
            <input type="number" class="form-control number-form-control ml-15 mr-15 J-edit-freight"
                   id="adjust_shipping_fee_modal" min="0" step="0.1" value="<?php echo $order_info['shipping_money'] - $order_info['promotion_free_shipping']; ?>">
...
<script>
    ...
    var order_info = <?php echo json_encode($order_info, JSON_UNESCAPED_UNICODE); ?>;
    var order_goods_list = <?php echo json_encode($order_goods_list, JSON_UNESCAPED_UNICODE); ?>;
