<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:71:"/www/wwwroot/www.tuandr.com/addons/shop/template/platform/shopList.html";i:1587970152;}*/ ?>
                                ...
                                <div class="v__control-group">
                                    ...
                                    <div class="v__controls">
                                        <select class="v__control_input" id="shop_type" >
                                            ...
                                            <?php if(is_array($shop_type_list) || $shop_type_list instanceof \think\Collection || $shop_type_list instanceof \think\Paginator): if( count($shop_type_list)==0 ) : echo "" ;else: foreach($shop_type_list as $key=>$vs): ?>
...
<div class="mb-20 bg-info border-info padding-15">店铺端登录地址：<a href="<?php echo __URL('ADMIN_MAIN/login','website_id='.$website_id); ?>" target="_blank"><?php echo __URL('ADMIN_MAIN/login','website_id='.$website_id); ?></a></div>

        <div class="mb-10">
            <a class="btn btn-primary" href="<?php echo __URL('ADDONS_MAINaddPlatformShop'); ?>"><i class="icon icon-add1"></i> 添加店铺</a>
        </div>
...
<script>
    require(['util'], function (util) {
        ...
        function LoadingInfo(page_index) {
            ...
            var url = '<?php echo $shopListUrl; ?>';
            $.ajax({
                type: "post",
                url: url,
                async: true,
                data: {
                    "page_index": page_index, "search_text": search_text, "shop_type": shop_type, "status": status, "website_id": '<?php echo $website_id; ?>'
                },
        ...
        $('#list').on('change','.J-sort',function(){
            ...
            $.ajax({
                type:"post",
                url:"<?php echo $updateShopListUrl; ?>",
