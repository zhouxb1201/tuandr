<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:84:"/www/wwwroot/www.tuandr.com/addons/areabonus/template/platform/areaBonusBalance.html";i:1583811698;}*/ ?>
		...
		<div class="flex-auto-center mb-20 bg-info text-center border-info">
			...
			<div class="flex-1 padding-15">
				<h3 class="strong">未发放分红的区域代理人数</h3>
				<p id="account_pay"></p>
			</div>
		</div>
...
<script>
    require(['util'],function(util){
        ...
        function getList(page_index) {
            ...
            $.ajax({
                type : "post",
                url : "<?php echo $areaBonusBalanceUrl; ?>",
                async : true,
                data : {
        ...
        $('.send').on('click',function(){
			...
			util.alert('是否发放分红？', function () {
                    $.ajax({
                        type: "post",
                        url: "<?php echo $areaBonusGrantUrl; ?>",
                        async: true,
                        success: function (data) {
                            if (data['code']) {
                                util.message('发放成功', 'success', "<?php echo __URL('ADDONS_MAINareaBonusBalance'); ?>");
                            } else {
