<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:75:"/www/wwwroot/www.tuandr.com/addons/delivery/template/platform/formList.html";i:1583811700;}*/ ?>
...
<div class="tab-content pt-15">
    <div class="tab-pane fade tab-1 active in" id="form_template">
        <div class="mb-20">
            <a href="<?php echo __URL('ADDONS_MAINaddFormTemplate'); ?>" class="btn btn-primary"><i class="icon icon-add1"></i> 添加模板</a>
        </div>
    ...
    <div class="tab-pane fade tab-3" id="delivery_template">
        <div class="mb-20">
            <a href="<?php echo __URL('ADDONS_MAINaddDeliveryTemplate'); ?>" class="btn btn-primary"><i class="icon icon-add1"></i>
                添加模板</a>
        </div>
        <table class="table v-table table-auto-center">
            <thead>
    ...
    <div class="tab-pane fade tab-4" id="sender_template">
        <div class="mb-20">
            <a href="<?php echo __URL('ADDONS_MAINaddSenderTemplate'); ?>" class="btn btn-primary"><i class="icon icon-add1"></i>
                添加模板</a>
        </div>
        <table class="table v-table table-auto-center">
            <thead>
...
<script>
    require(['util', 'tpl'], function (util, tpl) {
        ...
        $("#form_list").on('click', '.J-edit', function () {
            var id = $(this).parent('td').data('id')
            location.href = __URL('ADDONS_MAINupdateFormTemplate&id=' + id)
        })
        $("#sender_list").on('click', '.J-edit', function () {
            var id = $(this).parent('td').data('id')
            location.href = __URL('ADDONS_MAINupdateSenderTemplate&id=' + id)
        })
        $("#delivery_list").on('click', '.J-edit', function () {
            var id = $(this).parent('td').data('id')
            location.href = __URL('ADDONS_MAINupdateDeliveryTemplate&id=' + id)
        })
        $("#express_list").on('click', '.J-edit', function () {
            var id = $(this).parent('td').data('id')
            location.href = __URL('ADDONS_MAINupdateExpressTemplate&id=' + id)
        })
