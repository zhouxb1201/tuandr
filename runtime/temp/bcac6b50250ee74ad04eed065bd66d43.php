<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:86:"/www/wwwroot/www.tuandr.com/addons/microshop/template/platform/profitWithdrawList.html";i:1583811704;}*/ ?>
                ...
                <div class="filter-item clearfix">
                    <div class="filter-item__field">
                        ...
                        <div class="v__control-group">
                            <label class="v__control-label">用户名称</label>
                            <div class="v__controls">
                                <input type="text" id="search_text" class="v__control_input" placeholder="昵称、手机号、姓名" autocomplete="off">
                            </div>
                        </div>
...
require(['util'],function(util){
    ...
    function getWithdrawList(page_index){
        ...
        $.ajax({
            type: "post",
            url: "<?php echo $getWithdrawCountUrl; ?>",
        ...
        $.ajax({
            type : "post",
            url : "<?php echo $profitWithdrawListUrl; ?>",
            async : true,
            data : {
                ...
                if (data["data"].length > 0) {
                    for (var i = 0; i < data["data"].length; i++) {
                        ...
                        if(data["data"][i]["status"] == 1){
                            ...
                            html += '<a href="javascript:void(0);" class="btn-operation make_money" data-id="' + data["data"][i]["id"]+ '" data-type="' + data["data"][i]["type"]+ '">确认打款</a>';
                            html += '<a href="javascript:void(0);" class="btn-operation refuse_money" data-id="' + data["data"][i]["id"]+ '">拒绝打款</a>';
                        }else if(data["data"][i]["status"] == 5){
                            html += '<a href="javascript:void(0);" class="btn-operation make_money" data-id="' + data["data"][i]["id"]+ '" data-type="' + data["data"][i]["type"]+ '">重新打款</a>';
                            html += '<a href="javascript:void(0);" class="btn-operation refuse_money" data-id="' + data["data"][i]["id"]+ '">拒绝打款</a>';
                        }else{
    ...
    $('body').on('click','.check_all',function(){
        ...
        util.alert(title, function () {
            $.ajax({
                type: "post",
                url: "<?php echo $profitWithdrawInfoUrl; ?>",
                async: true,
                data: {
                    "id": check_id,
                    "status":status,
        ...
        $('.details').on('click',function(){
            ...
            $.ajax({
                ...
                success : function(list) {
                    ...
                    html += "<div class='col-md-8'>";
                    if(list['account_number']=='-1'){
                        html += "<p class='form-control-static'>用户余额</p>";
                    }else{
        ...
        $('.audit').on('click',function(){
            ...
            $.ajax({
                ...
                success : function(list) {
                    ...
                    html += "<div class='col-md-8'>";
                    if(list['account_number']=='-1'){
                        html += "<p class='form-control-static'>账户余额</p>";
                    }else{
                        html += "<p class='form-control-static'>"+list['account_number']+"</p>";
                    }
                    html += "</div>";
                    ...
                    util.confirm('审核通过', html,function(){
                        $.ajax({
                            type : "post",
                            url : "<?php echo $profitWithdrawAuditUrl; ?>",
                            async : true,
                            data : {
                                "id" : id,
                                "status":2
    ...
    $('.dataExcel').on('click',function(){
        ...
        window.location.href="<?php echo $profitWithdrawListDataExcelUrl; ?>&website_id="+website_id+"&status="+status+"&start_date="+start_date+"&end_date="+end_date+"&search_text="+search_text+"&withdraw_no="+withdraw_no;
    })
})
</script>
