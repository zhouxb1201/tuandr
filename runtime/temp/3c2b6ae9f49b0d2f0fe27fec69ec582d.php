<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:101:"/www/wwwroot/www.tuandr.com/addons/thingcircle/template/platform/thingcircleViolationCommentList.html";i:1583811702;}*/ ?>
            ...
            <ul class="nav nav-tabs v-nav-tabs add_tab1" role="tablist">
                <li role="presentation" <?php if(!$state): ?> class="active" <?php endif; ?>><a href="<?php echo __URL('platform/Menu/addonmenu?addons=violationCommentsList'); ?>" aria-controls="processed_comment" role="tab" class="flex-auto-center">待处理</a></li>
                <li role="presentation" <?php if($state == 2): ?> class="active" <?php endif; ?>><a href="<?php echo __URL('platform/Menu/addonmenu?addons=violationCommentsList&state=2'); ?>" aria-controls="ignored_comment" role="tab"class="flex-auto-center">已忽略</a></li>
                <li role="presentation" <?php if($state == 3): ?> class="active" <?php endif; ?>><a href="<?php echo __URL('platform/Menu/addonmenu?addons=violationCommentsList&state=3'); ?>" aria-controls="delete_comment" role="tab"class="flex-auto-center">已删除</a></li>
            </ul>
            <div class="mb-10">
                <?php if(!$state): ?>
...
<script>
    require(['util'],function(util){
        ...
        function getList(page_index){
            ...
            $.ajax({
                type : "post",
                url : '<?php echo $thingcircleViolationCommentListUrl; ?>',
                async : true,
                data : {
        ...
        $('body').on('click','.ign',function(){
            ...
            util.alert('是否忽略该举报？',function(){
                $.ajax({
                    type : "post",
                    url : "<?php echo $ignThingcircleCommentUrl; ?>",
        ...
        $('body').on('click','.del',function(){
            ...
            util.alert('是否删除该评论？',function(){
                $.ajax({
                    type : "post",
                    url : "<?php echo $delThingcircleCommentUrl; ?>",
        ...
        $('body').on('click','.delete',function(){
            ...
            util.alert('是否彻底删除该评论？',function(){
                $.ajax({
                    type : "post",
                    url : "<?php echo $deleteThingcircleCommentUrl; ?>",
                    data:{'comment_ids':commentId.toString()},
                    async : true,
                    success : function(data) {
                        if(data["code"] > 0 ){
        ...
        $('body').on('click','.rec',function(){
            ...
            util.alert('是否恢复该评论？',function(){
                $.ajax({
                    type : "post",
                    url : "<?php echo $recThingcircleCommentUrl; ?>",
                    data:{'comment_ids':commentId.toString()},
                    async : true,
                    success : function(data) {
                        if(data["code"] > 0 ){
