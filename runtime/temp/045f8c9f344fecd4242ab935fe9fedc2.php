<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:74:"/www/wwwroot/www.tuandr.com/addons/fullcut/template/admin/mansongList.html";i:1583811694;}*/ ?>
...
<div class="row addBtnSearch">
    <div class="col-sm-6 add">
        <a href="<?php echo __URL('ADDONS_ADMIN_MAINaddFullCut'); ?>" class="add_btn"><i class="icon icon-add1"></i> 添加活动</a>
    </div>
...
<script type="text/javascript">
  require(['utilAdmin','util'], function (utilAdmin,util) {
    ...
    function LoadingInfo(page_index) {
        ...
        $.ajax({
            type: "post",
            url: "<?php echo __URL('ADMIN_MAIN/Menu/addonmenu&addons=fullCutList&ajax=1'); ?>",
            data: {
                "shop_id": shop_id,
                ...
                if (data["data"].length > 0) {
                    for (var i = 0; i < data["data"].length; i++) {
                        switch (parseInt(data['data'][i]['status'])) {
                            case 0:
                                var status_name = '<span class="label label-success">未开启</span>';
                                break;
                            case 1:
                                var status_name = '<span class="label label-danger">进行中</span>';
                                break;
                            case 3:
    ...
    function closeMansong(mansong_id) {
        $.ajax({
            type: "post",
            url: "<?php echo __URL('admin/addons/execute/addons/fullcut/controller/fullcut/action/closemansong'); ?>",
    ...
    function delMansong(mansong_id) {
        ...
        $.ajax({
            type: "post",
            url: "<?php echo __URL('admin/addons/execute/addons/fullcut/controller/fullcut/action/delPromotionMansong'); ?>",
            data: {"mansong_id": mansong_id.toString()},
            dataType: "json",
            success: function (data) {
                if (data["code"] > 0) {
                    utilAdmin.message("删除成功","success", function () {
                        LoadingInfo($("#page_index").val());
                    })
                } else if (data["code"] == -1) {
