<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:84:"/www/wwwroot/www.tuandr.com/addons/thingcircle/template/admin/updateThingcircle.html";i:1586931646;}*/ ?>
...
<form class="form-horizontal widthFixedForm" role="form" id="form1">
	<div class="form-group">
		...
		<div class="col-md-8">
			<div>
				<label class="radio-inline"><input type="radio" name="thing_type" value="1" <?php if(($thing_info['thing_type'] == 1 || empty($thing_info['id']))): ?>checked<?php endif; ?>> 图文</label>
				<label class="radio-inline"><input type="radio" name="thing_type" value="2" <?php if(($thing_info['thing_type'] == 2)): ?>checked<?php endif; ?>> 视频</label>
			</div>
		</div>
	</div>
	...
	<div class="form-group">
		...
		<div class="col-md-8">
			<input type="text" class="form-control" name="thing_title" id="thing_title" autocomplete="off" value="<?php echo $thing_info['title']; ?>">
	...
	<div class="form-group">
		...
		<div class="col-md-8">
			<textarea class="form-control" name="content" id="content" rows="5" required><?php echo $thing_info['content']; ?></textarea>
		</div>
	</div>
	
	<div class="form-group pic-group <?php if(($thing_info['thing_type'] == 2)): ?>hide<?php endif; ?>">
		...
		<div class="col-md-8">
			<div id="J-goodspic" data-max="9">
				<div class="picture-list" id="J-pic">
					<?php if(($thing_info['img_temp_array'] && $thing_info['thing_type'] == 1)): foreach($thing_info["img_temp_array"] as $vo): ?>
					...
					<input type="hidden" id="upload_img_id" name="upload_img_id" value="<?php echo $vo['pic_id']; ?>" />
			...
			<input type="text" class="visibility" id="visibility1" name="picture" data-visi-type="multiPicture" <?php if((empty($thing_info['id']) || ($thing_info['thing_type'] == 1 && empty($thing_info['img_temp_array'])))): ?>required<?php endif; ?>>
		</div>
	</div>

	<div class="form-group video-group <?php if(($thing_info['thing_type'] == 1 || empty($thing_info['id']))): ?>hide<?php endif; ?>">
		<label class="col-md-2 control-label"><span class="text-bright">*</span>视频</label>
		<div class="col-md-8">
			<div class="picture-list" id="pc_video_adv">
        		<?php if(($thing_info['img_temp_array'] && $thing_info['thing_type'] == 2)): ?>
                   <a href="javascript:void(0);" class="close-box" style="margin-right:10px;">
                       <i class="icon icon-danger" style="right:-15px;" title="删除"></i>
                       <video width="80px" height="80px" src="<?php echo __IMG($thing_info['img_temp_array'][0]['pic_cover']); ?>"></video>
                   </a>
                <?php else: ?>
                   <a href="javascript:void(0);" class="plus-box" data-toggle="singleVideo"><i class="icon icon-plus"></i></a>
                <?php endif; ?>
                <input type="hidden" id="visibility2" name="upload_video_id" value="<?php echo $thing_info['media_val']; ?>" <?php if(($thing_info['thing_type'] == 2 && empty($thing_info['img_temp_array']))): ?>}required<?php endif; ?>/>
			</div>
			...
			<div class="border-default padding-15">
				<div class="mb-20">
					<div class="picture-list1">
						<?php if($recommend_goods_list): foreach($recommend_goods_list as $v): ?>
	...
	<div class="form-group">
		...
		<div class="col-md-5" id="selectTopic">
			<div class="input-group">
				<input type="text" class="form-control" id="topic" value="<?php echo $thing_info['topic_title']; ?>" disabled>
	...
	<div class="form-group">
		...
		<div class="col-md-5">
			<div class="input-group">
				...
				<input type="text" class="form-control w-200 inline-block" id="store_lat" name="store_lat" value="<?php echo $thing_info['lat']; ?>">
				<div class="input-group-addon">lng(经度)</div>
				<input type="text" class="form-control w-200 inline-block" id="store_lng" name="store_lng" value="<?php echo $thing_info['lng']; ?>">
			</div>
		</div>
	</div>
	...
	<div class="form-group">
		<label class="col-md-2 control-label"></label>
		<div class="col-md-8">
			<button class="btn btn-primary" type="submit" id="submitsData"><?php if(($thing_info['id']>0)): ?>保存<?php else: ?>添加<?php endif; ?></button>
			<a href="javascript:history.go(-1);" class="btn btn-default">返回</a>
		</div>
	</div>
	<input id="goodsid" name="goodsid" type="hidden" value="<?php echo $thing_info['recommend_goods']; ?>">
	<input type="hidden" id="id" name="id" value="<?php echo $thing_info['id']; ?>">
	<input type="hidden" id="topic_id" name="topic_id" value="<?php echo $thing_info['topic_id']; ?>">
</form>
<!--好物圈发布干货模板-->
...
<script>
	...
	require(['utilAdmin'], function (utilAdmin) {
		...
		$('#selectTopic').click(function () {
			utilAdmin.goodsDialog('url:'+'<?php echo $selectTopicList; ?>',function(data){
			});
		});
        var flag = false;
		utilAdmin.validate($('#form1'), function (form) {
            ...
            flag = true;
			$.post((id>0?'<?php echo $updateThingcircleUrl; ?>':'<?php echo $addThingcircleUrl; ?>'),data,
			function(res){
				if (res["code"] > 0) {
					utilAdmin.message((id>0?'保存成功':'添加成功'), 'success', "<?php echo __URL('ADDONS_ADMIN_MAINthingcircleList'); ?>");
				} else {
