<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:77:"/www/wwwroot/www.tuandr.com/addons/channel/template/platform/channelList.html";i:1583811706;}*/ ?>
                ...
                <div class="v__control-group">
                    ...
                    <div class="v__controls">
                        <select class="v__control_input" id="channel_grade">
                            ...
                            <?php if(is_array($channel_grade_list) || $channel_grade_list instanceof \think\Collection || $channel_grade_list instanceof \think\Paginator): if( count($channel_grade_list)==0 ) : echo "" ;else: foreach($channel_grade_list as $key=>$value): ?>
                            <option value="<?php echo $value['channel_grade_id']; ?>"><?php echo $value['channel_grade_name']; ?></option>
...
<ul class="nav nav-tabs v-nav-tabs fs-12" data-type="uncheck" id="check_now_time">
    <li role="presentation" class="active" data-status="2"><a href="javascript:;" class="flex-auto-center channel_status">全部<span class="J-all">(<?php echo $channel_count['all_count']; ?>)</span></a></li>
    <li role="presentation" class="" data-status="1"><a href="javascript:;" class="flex-auto-center channel_status">已审核<span class="J-all">(<?php echo $channel_count['checked_count']; ?>)</span></a></li>
    <li role="presentation" class="" data-status="0"><a href="javascript:;" class="flex-auto-center channel_status">待审核<span class="J-all">(<?php echo $channel_count['uncheck_count']; ?>)</span></a></li>
    <li role="presentation" class="" data-status="-1"><a href="javascript:;" class="flex-auto-center channel_status">审核不通过<span class="J-all">(<?php echo $channel_count['nocheck_count']; ?>)</span></a></li>
    <!--默认是2展示出所有的渠道商-->
        ...
        function getList(page_index){
            ...
            $.ajax({
                ...
                success : function(data) {
                    ...
                    var html = '';
                    if('<?php echo $page_status; ?>'=='down' && data["channel_info"]){
                    ...
                    if (data["data"].length>0) {
                        for (var i = 0; i < data["data"].length; i++) {
                            ...
                            if(data['data'][i]["status"] == 1){
                                html += '<td><span class="label label-green isexamine">已审核</span></td>';
                                ...
                                html += '<td><span class="label label-danger pass" data-type=1 data-channel_id="'+ data["data"][i]['channel_id']+'">审核未通过</span></td>';
                                ...
                                html += '<td><span class="label label-orange pass" data-type=1 data-channel_id="'+ data["data"][i]['channel_id']+'">待审核</span></td>';
        ...
        $('body').on('click','.save',function(){
            ...
            if(status==-1){
                util.alert('确定审核不通过？',function(){
                    $.ajax({
                        type : "post",
                        url : "<?php echo $channelCheckStatus; ?>",
